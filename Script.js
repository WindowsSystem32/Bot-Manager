      ...
      if (msg.startsWith(pre)) {
        ...
        if (msg == "목록") {
          ...
          if (iN(rSR)) {
            ...
            if (!(!iN(sS[sender]) && sSvd == false)) { //!(!iN(sS[sender]) && sSvd == false)
              sS[sender] = msg;
              sS2[sender] = rSR;
              sSvd[sender] = true;
            } else {
          ...
          replier.reply(lL3(sS[msg.substr(3)]));
        } else if (msg == "글로벌 로그") {
          ...
          if (!iN(sS[sender])) {
            replier.reply(sS2[sender].map((e, i) => "0".repeat(sS2[sender].length - (i + 1).length) + (i + 1) + "| " + e).join(ln));
          } else {
            replier.reply(aI[4] + "스크립트를 먼저 선택해 주세요!");
          }
        } else if (msg == "코드") {
          if (!iN(sS[sender])) {
            if (!iN(sL[sender])) {
              replier.reply(sL[sender] + "| " + sS2[sender][sL[sender]]);
            } else {
          ...
          if (!iN(sS[sender])) {
            var res5 = false;
            try {
              sS2[sender] = getSrc(msg).split(ln); //org.jsoup.Jsoup.connect(msg).get().select('body').html().split(ln);
              delete sL[sender];
              res5 = true;
            } catch(e) {}
            replier.reply(aI[(res5? 2 : 3)] + "'" + msg + "'(으)로부터 스크립트를 복제" + (res5? "했" : "하지 못했") + "습니다.");
          } else {
            replier.reply(aI[4] + "스크립트를 먼저 선택해 주세요!");
          }
...
function crSrc(name, apiLev, scriptPower, useUnifiedParams) {
  if (apiLev = 1 || apiLev == 2) {
    if (!name.includes(/[^0-9a-zA-Z ㄱ-ㅎㅏ-ㅣ가-힣]]/)) { //김유래님 감사합니다(/[^0-9a-zA-Z ㄱ-ㅎㅏ-ㅣ가-힣]]/)!
      ...
      " */" + ln + 
      "function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {" + ln + 
      "  " + ln + 
      "}" + ln + 
      ln + 
      ...
      ln + 
      "function onStop(activity) {}"; //레거시 API, 통합 X
      const dS2 = "const scriptName = \"" + name + "\";" + ln + 
      "/**" + ln + 
      " * (string) room" + ln + 
      " * (string) sender" + ln + 
      " * (boolean) isGroupChat" + ln + 
      ...
      "function onPause(activity) {}" + ln + 
      ln + 
      "function onStop(activity) {}"; //레거시 API, 통합됨
      const dS3 = "const bot = BotManager.getCurrentBot();" + ln +
      ln + 
      "/**" + ln +
      ...
      var f1 = ((apiLev == 1)? (useUnifiedParams? dS2 : dS1) : dS3);
      ...
      FileStream.write(sD(name) + "bot.json", f1);
      FileStream.write(sP(name), JSON.stringify(f2, 0, 1));
    } else {
